...
const Settings = () => {
    ...
    const teamMembers = [
        { name: 'Shweta Kore', role: 'Project Lead', initials: 'SK', status: 'online' },
        { name: 'Pratiksha Sawant', role: 'Frontend Developer', initials: 'PS', status: 'online' },
        { name: 'Rutuja Veer', role: 'Backend Developer', initials: 'RV', status: 'online' },
        { name: 'Sakshi Patil', role: 'UI/UX Designer', initials: 'SP', status: 'busy' },
        { name: 'Uday Pawar', role: 'Data Analyst', initials: 'UP', status: 'online' },
        { name: 'Siraj Sayyad', role: 'Full Stack Developer', initials: 'SS', status: 'online' },
    ];
        ...
        switch (activeTab) {
            case 'general':
                return (
                    <motion.div className="space-y-6" variants={containerVariants} initial="hidden" animate="visible">
                        <motion.div variants={itemVariants} className="p-5 rounded-xl bg-white/5 border border-white/10">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    {settings.darkMode ? <Moon size={20} className="text-purple-400" /> : <Sun size={20} className="text-amber-400" />}
                                    <div>
                        ...
                        <motion.div variants={itemVariants} className="p-5 rounded-xl bg-white/5 border border-white/10">
                            ...
                            {settings.autoRefresh && (
                                <div className="mt-4 pt-4 border-t border-white/5">
                                    <label className="text-xs text-gray-500 block mb-2">Refresh Interval</label>
                                    <select
            ...
            case 'notifications':
                return (
                    <motion.div className="space-y-6" variants={containerVariants} initial="hidden" animate="visible">
                        {[
                            { key: 'notifications', icon: Bell, title: 'Push Notifications', desc: 'Receive in-app notifications' },
                            { key: 'emailAlerts', icon: Mail, title: 'Email Alerts', desc: 'Receive alert emails for critical issues' },
                            { key: 'smsAlerts', icon: Smartphone, title: 'SMS Alerts', desc: 'Receive SMS for emergency alerts' },
                        ].map((item) => (
                            <motion.div key={item.key} variants={itemVariants} className="p-5 rounded-xl bg-white/5 border border-white/10">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                    ...
                    <motion.div className="space-y-6" variants={containerVariants} initial="hidden" animate="visible">
                        ...
                        <motion.div variants={itemVariants} className="p-5 rounded-xl bg-white/5 border border-white/10">
                            ...
                            <div className="space-y-3">
                                <input type="password" placeholder="Current Password" className="w-full px-3 py-2 rounded-lg bg-black/30 border border-white/10 text-white text-sm focus:outline-none focus:border-cyan-500/50" />
                                <input type="password" placeholder="New Password" className="w-full px-3 py-2 rounded-lg bg-black/30 border border-white/10 text-white text-sm focus:outline-none focus:border-cyan-500/50" />
                                <input type="password" placeholder="Confirm New Password" className="w-full px-3 py-2 rounded-lg bg-black/30 border border-white/10 text-white text-sm focus:outline-none focus:border-cyan-500/50" />
                                <button
                    ...
                    <motion.div className="space-y-6" variants={containerVariants} initial="hidden" animate="visible">
                        <motion.div variants={itemVariants} className="p-5 rounded-xl bg-white/5 border border-white/10">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <div>
                                    <h4 className="text-sm font-bold text-white">Data Retention</h4>
                                    <p className="text-xs text-gray-500">Set how long to keep historical data</p>
                                </div>
                            </div>
                            <select
                        ...
                        <motion.div variants={itemVariants} className="p-5 rounded-xl bg-white/5 border border-white/10">
                            <h4 className="text-sm font-bold text-white mb-4">Storage Usage</h4>
                            <div className="space-y-3">
                                <div>
            ...
            case 'integrations':
                return (
                    <motion.div className="space-y-6" variants={containerVariants} initial="hidden" animate="visible">
                        {[
                            { name: 'SCADA System', status: 'connected', desc: 'Real-time sensor integration' },
                            { name: 'Weather API', status: 'connected', desc: 'Weather data for predictions' },
                            { name: 'SMS Gateway', status: 'disconnected', desc: 'SMS alert delivery' },
                            { name: 'Cloud Backup', status: 'connected', desc: 'Automated data backup' },
                        ].map((integration, i) => (
                            <motion.div key={i} variants={itemVariants} className="p-5 rounded-xl bg-white/5 border border-white/10 flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    <div className={`w-10 h-10 rounded-xl flex-center ${integration.status === 'connected' ? 'bg-green-500/10 border border-green-500/30' : 'bg-red-500/10 border border-red-500/30'}`}>
                                        <Wifi size={18} className={integration.status === 'connected' ? 'text-green-400' : 'text-red-400'} />
                                    </div>
                                    <div>
                                ...
                                <div className="flex items-center gap-3">
                                    <span className={`text-xs px-2 py-1 rounded-full ${integration.status === 'connected' ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}`}>
                                        {integration.status}
                                    </span>
                        ...
                        <motion.div variants={itemVariants} className="p-5 rounded-xl bg-white/5 border border-white/10">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500">AquaTracker development team</p>
                                </div>
                            </div>
                        </motion.div>
                                ...
                                >
                                    <div className="flex items-center gap-4">
                                        <div className="relative">
                                            ...
                                            <div className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-gray-900 ${member.status === 'online' ? 'bg-green-500 shadow-[0_0_8px_#22c55e]' :
                                                member.status === 'busy' ? 'bg-amber-500 shadow-[0_0_8px_#f59e0b]' :
                                                    'bg-gray-500'
                                                }`} />
                                        </div>
        ...
        >
            ...
            <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                    ...
                    <p className="text-sm text-gray-500 font-mono">Configure your preferences</p>
                </div>
            </div>
