...
const Reports = () => {
    ...
    const { exportToCSV, exportToJSON, generateReport, isExporting, progress } = useExport();

    const reports = [
        { id: 1, title: 'December 2025 Compliance Report', type: 'compliance', date: 'Jan 5, 2026', status: 'completed', size: '2.4 MB', pages: 24 },
        { id: 2, title: 'Q4 2025 Water Quality Summary', type: 'quarterly', date: 'Jan 3, 2026', status: 'completed', size: '5.1 MB', pages: 48 },
        { id: 3, title: 'Weekly Operations Report - W1', type: 'weekly', date: 'Jan 2, 2026', status: 'completed', size: '1.2 MB', pages: 12 },
        { id: 4, title: 'Incident Report - Station C1 pH Alert', type: 'incident', date: 'Jan 1, 2026', status: 'pending', size: '0.8 MB', pages: 8 },
        { id: 5, title: 'November 2025 Compliance Report', type: 'compliance', date: 'Dec 5, 2025', status: 'completed', size: '2.2 MB', pages: 22 },
        { id: 6, title: 'Annual Audit Report 2025', type: 'audit', date: 'Dec 1, 2025', status: 'completed', size: '8.5 MB', pages: 86 },
        { id: 7, title: 'Equipment Maintenance Log Q4', type: 'maintenance', date: 'Nov 28, 2025', status: 'completed', size: '3.2 MB', pages: 34 },
        { id: 8, title: 'Environmental Impact Assessment', type: 'audit', date: 'Nov 15, 2025', status: 'draft', size: '4.8 MB', pages: 52 },
    ];
    ...
    const getStatusIcon = (status) => {
        switch (status) {
            ...
            default: return <FileClock size={14} className="text-gray-400" />;
        }
    };
        ...
        >
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold font-display">Compliance Reports</h1>
                        <p className="text-sm text-gray-500 font-mono">{reports.length} total reports</p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                    { label: 'Total Reports', value: reports.length, icon: FileText, color: 'cyan' },
                    { label: 'Completed', value: reports.filter(r => r.status === 'completed').length, icon: CheckCircle, color: 'green' },
                    { label: 'Pending', value: reports.filter(r => r.status === 'pending').length, icon: Clock, color: 'amber' },
                    ...
                    >
                        <stat.icon size={20} style={{ color: stat.color === 'cyan' ? '#00f2ff' : stat.color === 'green' ? '#22c55e' : stat.color === 'amber' ? '#f59e0b' : '#bd00ff' }} />
                        <div className="text-2xl font-bold text-white font-display mt-2">{stat.value}</div>
                ...
                <div className="divide-y divide-white/5">
                    {filteredReports.map((report, index) => (
                        ...
                        >
                            ...
                            <span className={`flex items-center gap-1.5 text-[10px] px-2.5 py-1 rounded-full border ${getStatusStyle(report.status)}`}>
                        ...
                        >
                            ...
                            >
                                <div className="flex items-center justify-between mb-6">
                                    <h3 className="text-lg font-bold font-display">Export Reports</h3>
                                    <button
                                        onClick={() => setShowExportModal(false)}
                                ...
                                <div className="space-y-3">
                                    <motion.button
                                        onClick={() => {
                                            exportToCSV(filteredReports.map(r => ({
                                                Title: r.title,
                                                Type: r.type,
                                                Date: r.date,
                                                Status: r.status,
                                    ...
                                    >
                                        ...
                                        <div className="text-left">
                                            <div className="text-sm font-bold text-white group-hover:text-green-400 transition-colors">Export as CSV</div>
                                    ...
                                    >
                                        ...
                                        <div className="text-left">
                                            <div className="text-sm font-bold text-white group-hover:text-amber-400 transition-colors">Export as JSON</div>
                                            <div className="text-xs text-gray-500">Data interchange format</div>
                                        </div>
                                    </motion.button>
                                                ...
                                                sections: [
                                                    {
                                                        ...
                                                        data: [
                                                            ...
                                                            { label: 'Completed', value: filteredReports.filter(r => r.status === 'completed').length },
                                                            { label: 'Pending', value: filteredReports.filter(r => r.status === 'pending').length },
                                                        ]
                                                    },
                                                    {
                                                        title: 'Report List',
                                                        type: 'table',
                                                        headers: ['Title', 'Type', 'Date', 'Status', 'Pages'],
                                                        rows: filteredReports.map(r => [r.title, r.type, r.date, r.status, r.pages])
                                                    }
                                                ]
                                            });
                                            setShowExportModal(false);
                                    ...
                                    >
                                        ...
                                        <div className="text-left">
                                            <div className="text-sm font-bold text-white group-hover:text-cyan-400 transition-colors">Generate Print Report</div>
                                            <div className="text-xs text-gray-500">Formatted HTML for printing</div>
                                        </div>
                                    </motion.button>
                                </div>
